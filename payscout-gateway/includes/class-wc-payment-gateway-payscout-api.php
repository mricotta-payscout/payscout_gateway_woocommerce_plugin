...
class WC_Payscout_API {
	...
	public static function confirm_payment_intent( $id, $post = array() ) {
		...
		if ( ! empty( $secret_key ) ) {
			...
			if ( ( ! $response instanceof WP_Error ) && 200 === $response['response']['code'] ) {
				return $response;
			} elseif ( isset( $response['body'] ) && isset( json_decode( $response['body'] )->error ) && isset( json_decode( $response['body'] )->error->message ) && ( json_decode( $response['body'] )->error->message === 'Invalid payment intent status: processing' || json_decode( $response['body'] )->error->message === 'Invalid payment intent status: succeeded' ) ) {
